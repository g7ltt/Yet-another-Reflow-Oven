...
SMD Reflow Oven Controller
  ...
  Modifications by Mark Phillips, G7LTT

Version V2.0a 20150321
...
Three profiles embedded in the code below L, M & H. Press MENU when run has started to change.
...
REVISIONS:
  Pins reallocated from Ian's original code to suit G7LTT YARO board
...
LiquidCrystal lcd(7, 8, 9, 12, 10, 11); // Pin assignment of LCD interface: RS, ENABLE, D4, D5, D6, D7
...
void setup() {
	...
	// Set up pins
	pinMode(DI_StopSW, INPUT_PULLUP);
	pinMode(DI_StartSW, INPUT_PULLUP);
	pinMode(DI_ModeSW, INPUT_PULLUP);
	...
	digitalWrite(DO_Relay, HIGH);   // Turn the relay off - connected to PNP transistor
	digitalWrite(DO_Buzz, LOW);    // Turn the buzzer off
        digitalWrite(DO_LED, LOW);     // Turn of the LED
	...
	delay(1500);
        ...
        digitalWrite(DO_LED, HIGH);  // Turn the LED on
        digitalWrite(DO_Buzz, HIGH);  // Turn on buzzer
        delay(150);
        digitalWrite(DO_LED, LOW);  // Turn the LED off
...
void tickslowmS() {  
    ...
    if(++tickslow == 5) { // 5 * 200 = 1000mS
 	...
 	if(Status == 0) {  // Stop or finished mode
	    digitalWrite(DO_Relay, HIGH);   // Turn the relay off
            digitalWrite(DO_LED, LOW);  // Turn the LED off
    ...
    if(Status == 3) {  // Oven is now go
		lcd.setCursor(0,1); // column, row
		lcd.print(DegC); lcd.print("  ");lcd.print((char)223);
		lcd.setCursor(5,1); // column, row
        lcd.print(Seconds);
        ...
        if(++tickslow2 == 10) { // 2 * 1000 = 2000mS
            ...
            if(DegC > DegCtempstep) {  // If temperature is too high then turn off heater
	            digitalWrite(DO_Relay, HIGH);
                    digitalWrite(DO_LED, LOW);  // Turn the LED ff
                lcd.setCursor(7,0); // column, row
                lcd.print(" "); // indicate heaters off
			}
        }		
